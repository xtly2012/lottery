...
public class LotteryInitTimeServiceImpl implements LotteryInitTimeService
{
    ...
    public Date getLotteryInitTime()
    {
        ...
        LotteryInitTime maxInitTime = this.lotteryInitTimeDao.queryMaxInitTime();
        if (maxInitTime != null && maxInitTime.getLotteryInitTime().getTime() >= calendar.getTime().getTime())
        {
            return null;
        }
